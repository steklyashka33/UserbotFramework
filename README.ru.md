...
> [!NOTE]
> Остальные параметры (порты, флаги функций) настраиваются напрямую в [shared/config.py](shared/config.py).

### 🌐 Сетевые особенности и доступность
В некоторых регионах прямой доступ к серверам API Telegram может быть затруднен или нестабилен. В таких случаях для корректной работы системы рекомендуется:
*   Использовать надежные **прокси-сервера** (MTProto).
*   Запускать проект через **VPN-сервисы** на уровне хоста.
*   Разворачивать инфраструктуру на серверах в регионах с беспрепятственным доступом к глобальной сети.
